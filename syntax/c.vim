...
if exists("c_curly_error")
  ...
  syn region	cBlock		start="\%(\^\%([^{]\+\)\=\s*\)\@<!{" end="}" contains=ALLBUT,cBadBlock,cCurlyError,@cParenGroup,cErrInParen,cCppParen,cErrInBracket,cCppBracket,cCppString,@Spell fold
  syn region	cAppleBlock	start="\^\%([^{]\+\)\=\s*{" end="}" contains=ALLBUT,cBadBlock,cCurlyError,@cParenGroup,cErrInParen,cCppParen,cErrInBracket,cCppBracket,cCppString,@Spell fold
else
  syn region	cBlock		start="\%(\^\%([^{]\+\)\=\s*\)\@<!{" end="}" transparent fold
  syn region	cAppleBlock	start="\^\%([^{]\+\)\=\s*{" end="}" transparent
endif
...
if !exists("c_no_if0")
  ...
  syn region	cCppInIf	contained matchgroup=XcCppInWrapper start="\d\+" end="^\s*\(%:\|#\)\s*endif\>" contains=TOP,cPreCondit
  if !exists("c_no_if0_fold")
